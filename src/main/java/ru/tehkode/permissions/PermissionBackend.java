...
public abstract class PermissionBackend {
	...
	public static Class<? extends PermissionBackend> getBackendClass(String alias) throws ClassNotFoundException {
		Class<? extends PermissionBackend> backendClass = registeredAliases.get(alias);

		if (backendClass == null) {
			Class<?> clazz = Class.forName(alias);
			if (!PermissionBackend.class.isAssignableFrom(clazz)) {
				throw new ClassNotFoundException("Class " + alias + " does not extend PermissionBackend");
			}
			backendClass = (Class<? extends PermissionBackend>) clazz;
		}
	...
	public static String getBackendAlias(Class<? extends PermissionBackend> backendClass) {
		if (registeredAliases.containsValue(backendClass)) {
			for (String alias : registeredAliases.keySet()) { // Is there better way to find key by value?
